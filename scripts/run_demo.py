...
if __name__ == "__main__":
    ...
    def main():
        ...
        print(f"\n✨ {hero.name} appears!")
        print(f"   HP: {hero.hp}/{hero.max_hp} | ATK: {hero.attack} | DEF: {hero.defense}")
        ...
        print(f"   HP: {goblin.hp}/{goblin.max_hp} | ATK: {goblin.attack} | DEF: {goblin.defense}")
        
        print(f"\n⚔️  {hero.name} attacks {goblin.name}!")
        damage = resolve_attack(hero, goblin)
        print(f"   Dealt {damage} damage! {goblin.name} HP: {goblin.hp}/{goblin.max_hp}")
        ...
        print(f"   Currency: ${hero.currency}")
        print(f"   Bank Balance: ${bank.check_balance(hero)}")
        print(f"   Inventory: {', '.join(item.name for item in hero_inventory.list_items())}")
        print("\n" + "=" * 60)
        print("Demo Complete!")
        print("=" * 60)
